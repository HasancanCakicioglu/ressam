...
import 'package:ressam/Flame/girl_skater/world/ground.dart';
import 'package:ressam/Flame/girl_skater/world/intro.dart';
...
class LeenaGame extends FlameGame with HasCollisionDetection, TapDetector {
  ...
  Future<void> onLoad() async {
    ...
    // camera.viewport = FixedResolutionViewport(Vector2(size[0]*3, mapHeight));
  ...
  void onTapDown(TapDownInfo info) {
    ...
    if (leena.onGround && introFinished) {
      if (info.eventPosition.viewport.x < 300) {
        timerStarted = true;
        print('push left');
      ...
      } else if (info.eventPosition.viewport.x > size[0] - 300) {
        timerStarted = true;
        print('push right');
        if (!leena.facingRight) {
      ...
      if (info.eventPosition.game.y < 200) {
        print('jump up');
